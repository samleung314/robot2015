...
public class TurnPID implements PIDOutput {
	...
	public TurnPID(Robot robot) {
		...
		/* Experimental */
		doubleTurnPIDA = new PIDController(dtP, dtI, dtD, gyro,
				robot.drive.leftVics);
		doubleTurnPIDA.setContinuous();
		doubleTurnPIDA.setOutputRange(-0.5, 0.5);

		doubleTurnPIDB = new PIDController(dtP, dtI, dtD, gyro,
				robot.drive.rightVics);
		doubleTurnPIDB.setContinuous();
		doubleTurnPIDB.setOutputRange(-0.5, 0.5);
	}

	public boolean turnDegrees(double degrees, double turnSpeed) { // Method for
																	// making
																	// robot
																	// turn a
																	// number of
		...
		if (gyro.getAngle() < degrees - 2 || gyro.getAngle() > degrees + 2) { // +-
			...
			if (degrees < 0) { // Negative Degrees (CCW Direction)
				if (gyro.getAngle() > degrees) {
					robot.drive.doubleVicsDrive(-turnSpeed, turnSpeed); // Turn
																		// CCW
	...
	public boolean PIDturnDegrees(double degrees) { // Method for making robot
	...
	/* Experimental */
	public void doublePIDturnDegrees(double degrees) { // Method for making
														// robot turn a number
														// of degrees using two
														// PID controllers
	...
	@Override
	// Returns output speed for robot to turn at
